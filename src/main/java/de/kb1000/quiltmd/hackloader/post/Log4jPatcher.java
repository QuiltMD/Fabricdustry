 ...
 */
package de.kb1000.quiltmd.hackloader.post;

import de.kb1000.quiltmd.hackloader.annotations.NotNull;
import de.kb1000.quiltmd.hackloader.annotations.Nullable;
...
public class Log4jPatcher {
    private static final @NotNull String JMX_SERVER_CLASS = "org/apache/logging/log4j/core/jmx/Server";

    public static void patch() throws IOException, NoSuchMethodException, InvocationTargetException, IllegalAccessException {

        final @NotNull ClassLoader contextClassLoader = Thread.currentThread().getContextClassLoader();
        try (final @Nullable InputStream is = contextClassLoader.getResourceAsStream("org/apache/logging/log4j/core/LoggerContext.class")) {
            if (is == null) {
                throw new NoClassDefFoundError("org.apache.logging.log4j.core.LoggerContext");
            }

            final @NotNull ClassReader classReader = new ClassReader(is);
            final @NotNull ClassWriter classWriter = new ClassWriter(0);
            classReader.accept(new ClassVisitor(ASM9, classWriter) {
                @Override
                public @Nullable FieldVisitor visitField(int access, @NotNull String name, @NotNull String descriptor, @Nullable String signature, @Nullable Object value) {
                ...
                @Override
                public @Nullable MethodVisitor visitMethod(int access, @NotNull String name, @NotNull String descriptor, @Nullable String signature, @Nullable String[] exceptions) {
                    if (name.equals("firePropertyChangeEvent") || name.equals("addPropertyChangeListener") || name.equals("removePropertyChangeListener")) {
                        return null;
                    }
                    final @Nullable MethodVisitor methodVisitor = super.visitMethod(access, name, descriptor, signature, exceptions);
                    if (name.equals("<init>")) {
